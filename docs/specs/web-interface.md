...
| 8c | Suggestion | One folder holding coding sessions that no registered project covers | its folder name, its full path, the number of sessions in it, and a one-press register control (per R9) |
...
- **Suggested projects:** under the list, one entry per folder where a coding
  session is running that no registered project covers — its folder name, its
  full path, how many sessions are in it, and a control that registers it in
  one press, going through the same registration as the field above and
  meeting the same refusals. A folder already inside a registered project is
  ...
  walks up through a parent. Sessions with no agent count the same as agents.
  Two sessions in one folder are one entry. The block follows the terminal
  switch alone and shows nothing when it is off (per R6, R9).
