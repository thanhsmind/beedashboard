...
**`gate_bypass` is not in `state.json`.** It lives in `.bee/config.json`, a file the reader opens today for nothing at all — observed as `false` here and `"total"` in beehive, so it is read defensively and normalized. Reading it needs a new reader and a new fixture builder, not just a new field.
...
| **E3 — What needs attention** | D6's generated rules, and the review/debt readers that feed them | This is what makes it a management view rather than a data dump | S1, S3 | Each rule fires and stays silent under fixtures built for it; a fixture tripping several rules at once orders them heaviest-first; every emitted item carries a suggested action; an empty list says so in one line |
...
|---|---|---|
| **S1** (current) | The free-text scrubber and its tests (D9). Reader learns `approved_gates`, `route`, `next_action` from `state.json`, `handoff` from `.bee/HANDOFF.json`, and `gate_bypass` from `.bee/config.json` — the last two each needing a new reader and a new fixture builder. New page skeleton: header, lifecycle stepper (D7-correct), KPI row, "working on now" card, attention panel carrying only the rules S1's data supports (handoff, blocked cells, read errors, bypass not off). The old sections remain below, untouched. | — |
| **S2** | Lane records read with their own phase/gates/created_at (`lane_json` must grow both fields first); per-feature cell counts; the by-phase board replaces **both** the four-bucket section and the existing lanes panel (`bee_lanes_panel`, `views.rs:1357`), which otherwise renders the same features a second time; cards link to detail pages (D3). | S1 |
| **S3** | Review join (`review-candidates.jsonl` × `.bee/reviews/*.json`), capture queue, scribing-debt derivation, promote-proposal presence. Attention rules extended to open P1s, unreviewed high-risk, knowledge debt. Review/backlog panel. | S1 |
| **S4** | Velocity folded in as the delivery-speed block; worktrees and workspaces folded into the sessions panel; the old running-now section (`bee_running_now_section`, `views.rs:739`) retired into S1's "working on now" card, which by then carries its whole job; process-health panel (tier mix, reservations, gate bypass, read errors). | S1, S2 |
...
**Current slice to prepare: S1 only.** It is a walking skeleton by construction: real store data, real page, no stubs, and the old sections keep working beneath it, so the page is never half-built in the browser.

**S1's proof is not only its own new tests.** Three existing tests assert over the *whole* response body, so S1's markup falls inside their scope even though S1 changes none of their sections. S1 is not green until all three still pass, and each names a way the new sections could break them:

- `no_shipped_features_renders_honest_empty_state_not_zeros` (`server.rs:2731`) forbids the substrings `0.0` and `0/0` anywhere in the body. Its fixture has two cells, nothing shipped, and **no `state.json`** — precisely the state where a naive progress bar emits `width: 0.0%` or a KPI reads `0/0`. S1's honest-empty-state handling is what this test measures.
...
**Reviewed before the gate.** An independent pass checked every anchor and Discovery claim in this plan against the repo and found one P1 — the D9 free-text gap now written into Approach, the risk map and the test matrix — plus two P2s (the duplicate lanes panel and running-now section nothing had retired; S1's three whole-body tests) and eight P3 corrections, all applied above. Its confirmations are load-bearing too: the test taxonomy (12 invariants / 7 detail-page / 28 markup / 45 reader) and all fifteen rule anchors in the table above were verified individually.
...
| Side effects | The fixture `.bee` tree is byte-identical after every request to the board | **Already pinned — must not regress** |
